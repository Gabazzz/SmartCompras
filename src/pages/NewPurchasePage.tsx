...
export default function NewPurchasePage() {
  ...
  const handleProdutoChange = (valor: string) => {
    ...
    const historico = compras.find((c) => c.produto.toLowerCase() === valor.toLowerCase());
    if (historico) {
      setCategoria(historico.categoria);
      return;
    }

    // 2) Tenta o dicionário de produtos comuns
    const detectada = detectarCategoria(valor);
    if (detectada) {
      setCategoria(detectada);
    } else if (valor.trim() === '') {
      setCategoria('');
    }
  };

  const handleCategoriaManual = (cat: Categoria) => {
    setCategoria(cat);
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  ...
                  {categoria && categoriaAuto && (
                    <span className="text-[9px] font-label px-1.5 py-0.5 rounded-full" style={{ color: '#00F5FF', background: 'rgba(0,245,255,0.1)', border: '1px solid rgba(0,245,255,0.25)' }}>
                      sugerido
                    </span>
                  )}
                </div>
                <div className="flex flex-wrap gap-2">
                  {CATEGORIAS.map((cat) => {
                    ...
                    return (
                      <button key={cat} type="button" onClick={() => handleCategoriaManual(cat)}
